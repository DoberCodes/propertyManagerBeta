...
const createMockWebhookEvent = (eventType: string, data: any) => ({
...
export const testCancelSubscriptionEndpoint = async () => {
	...
	try {
		...
		const response = await fetch(TEST_CONFIG.cancelSubscriptionUrl, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
		...
		console.error(
			'❌ Cancel subscription test failed:',
			error instanceof Error ? error.message : String(error),
		);
		return false;
	}
...
export const testDeleteAccountEndpoint = async () => {
	console.log('🗑️ Testing delete account endpoint...');

	try {
		const response = await fetch(TEST_CONFIG.deleteAccountUrl, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
...
export const testStripeWebhookSignatureVerification = async () => {
	...
	try {
		...
		if (response.status === 400) {
			console.log(
				'✅ Webhook signature verification is working (rejected invalid signature)',
			);
			return true;
		} else {
			console.log(
				'⚠️ Webhook signature verification may not be working properly',
			);
			return false;
		}
	} catch (error) {
...
export const runIntegrationTests = async () => {
	...
	console.log(
		'⚠️  Note: These tests verify endpoint connectivity and basic functionality.',
	);
	console.log(
		'⚠️  Full functionality testing requires valid authentication tokens.\n',
	);
	...
	const tests = [
		...
		{ name: 'Delete Account Endpoint', fn: testDeleteAccountEndpoint },
		{
			name: 'Webhook Signature Verification',
			fn: testStripeWebhookSignatureVerification,
		},
	];
